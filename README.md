...
3.  **پیکربندی متغیرهای محیطی**:
    ...
    - مقدار `API_BASE_URL` را متناسب با آدرس سرور بک‌اند خود تنظیم کنید.
    - در صورت نیاز به چند محیط، می‌توانید نسخه‌های مختلفی از `.env` ایجاد کرده و قبل از build، فایل مناسب را جایگزین کنید. همچنین امکان override مقدار با استفاده از `--dart-define=API_BASE_URL=...` وجود دارد.
4.  **اجرای برنامه**:
